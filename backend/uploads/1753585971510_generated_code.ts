...
export class LandingPageHelper {
  ...
  async getHeaderTitle(): Promise<string> {
    this.logger.info('Getting header title');
    try {
      const element = await this.driver.findElement(this.title);
      const text = await element.getText();
      this.logger.info('Header title retrieved successfully');
      return text;
    } catch (error) {
      this.logger.error('Error getting header title', error);
      throw error;
    }
  }

  async getSubtitlesText(): Promise<string> {
    this.logger.info('Getting subtitles text');
    try {
      const element = await this.driver.findElement(this.subtitles);
      const text = await element.getText();
      this.logger.info('Subtitles text retrieved successfully');
      return text;
    } catch (error) {
      this.logger.error('Error getting subtitles text', error);
      throw error;
    }
  }
  ...
  async fillPackingSlipField(value: string): Promise<void> {
  ...
  async getGridHeaderTitle(): Promise<string> {
    ...
    try {
      ...
      const text = await element.getText();
      this.logger.info('Grid header title retrieved successfully');
      return text;
    } catch (error) {
